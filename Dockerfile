...
RUN wget --no-check-certificate  https://nodejs.org/dist/latest-v8.x/node-v8.9.4-linux-x64.tar.xz
#RUN tar xvf node-${node_version}-linux-x64.tar.xz
RUN tar xvf node-v8.9.4-linux-x64.tar.xz

ENV PATH /opt/bin:$PATH

# RUN ln -s /opt/src/node-v9.5.0-linux-x64/bin /opt/bin
RUN ln -s /opt/src/node-v8.9.4-linux-x64/bin /opt/bin
